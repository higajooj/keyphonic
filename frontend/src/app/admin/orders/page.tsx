    ...
    <div className="flex grow flex-col gap-8">
        ...
        <div className="flex flex-col justify-start rounded-xl border p-8 pb-0 sm:min-w-[412px]">
          ...
          <div className="flex grow flex-col justify-between gap-2 py-4 xl:justify-evenly xl:py-0">
            {statisticsData.map(({ count, label, value }) => (
              <div className="flex flex-col justify-between sm:flex-row sm:items-end" key={label}>
                <span>
      ...
      <div className="flex grow flex-col gap-4">
        <Input
          ...
          variant="search"
        />
        <DataTable columns={columns} data={data} onClickRow={(o) => handleSelectOrder(o.id)} />
        <SheetDetailOrder onOpenChange={setOpenDetails} open={openDetails} orderId={selectedOrderId} />
      </div>
    </div>
  );
}
